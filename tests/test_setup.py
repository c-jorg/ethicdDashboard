...
def test_setup_guest(client, db_session):
    ...
    THEN response should contain success message
    """
    db_session.query(CaseStudy).filter(CaseStudy.title == "Guest Case Study").delete()
    ...
    print(response)
    assert response.status_code ==  201
    assert db_session.query(CaseStudy).filter(CaseStudy.title == "Guest Case Study").count() == 1
    assert db_session.query(CaseStudy).filter(CaseStudy.title == "Guest Case Study 2").count() == 1
    assert db_session.query(Class).filter(Class.class_name == "guest class").count() == 1
    assert db_session.query(Class).filter(Class.class_name == "guest class 2").count() == 1
